  ...
  <div class="wrapper">
    <form @submit.prevent="submitHandler">
      ...
      <TextInput
        ...
        label="Ваша должность (при необходимости укажите новую)"
      />
      ...
      <MainSelect
        ...
        :options="categories"
        label="Категория персонала (при необходимости укажите новую) "
        :required="true"
      />
      <MainSelect
        class="select"
...
watch(
  ...
  async (userName) => {
    ...
    if (user) {
      ...
      previousGroup.value = user[0].previousGroup;
      previousDate.value = user[0].previousDate;
      store.commit("updatePreviousGroup", user[0].group);
